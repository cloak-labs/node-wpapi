...
const expectedResults = {
	names: {
		...
		pageLast: ["ween", "wellhead", "wellintentioned", "whetstone", "years"],
	},
};
	...
	describe("paging properties", () => {
		...
		it("provides a bound WPRequest for the next page as .next", () => {
			const prom = wp
				...
				.then((categories) => {
					...
					expect(categories._paging.next._options.endpoint).toEqual(
						"http://wpapi.local/wp-json/wp/v2/categories?page=2"
					);
					...
					return wp
						...
						.then((categories) => {
							...
							expect(getNames(categories)).toEqual(
								expectedResults.names.pageLast
							);
							return SUCCESS;
						});
				});
		...
		it("allows access to the next page of results via .next", () => {
			const prom = wp
				...
				.get()
				.then((categories) => {
					return categories._paging.next.get().then((categories) => {
						expect(Array.isArray(categories)).toBe(true);
						expect(categories.length).toBe(10);
			...
			const prom = wp
				...
				.then((categories) => {
					...
					return categories._paging.next.get().then((categories) => {
						...
						expect(categories._paging.prev._options.endpoint).toEqual(
							"http://wpapi.local/wp-json/wp/v2/categories?page=1"
						);
						return SUCCESS;
					});
				});
		...
		it("allows access to the previous page of results via .prev", () => {
			const prom = wp
				...
				.then((categories) => {
					expect(getNames(categories)).toEqual(expectedResults.names.page2);
					return categories._paging.prev.get().then((categories) => {
						expect(Array.isArray(categories)).toBe(true);
						expect(categories.length).toBe(10);
			...
			const prom = wp
				...
				.then((categories) => {
					...
					const slugs = categories
						...
						.join(" ");
					expect(slugs).toBe(
						"foo-a-foo-parent foo-parent parent parent-category"
					);
					return SUCCESS;
				});
			return expect(prom).resolves.toBe(SUCCESS);
			...
			const prom = wp
				...
				.then((categories) => {
					...
					categories.forEach((cat, idx) => {
						["id", "name", "slug", "taxonomy"].forEach((prop) => {
