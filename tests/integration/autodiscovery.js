...
describe.each([
	...
	it("eventually returns a configured WP instance", () => {
		const prom = apiPromise.then((result) => {
	...
	it("auto-binds to the detected endpoint on the provided site", () => {
		const prom = apiPromise.then((site) => {
			expect(site.posts().toString()).toBe(
				"http://wpapi.local/wp-json/wp/v2/posts"
			);
			return SUCCESS;
		});
		return expect(prom).resolves.toBe(SUCCESS);
	...
	it("resolves to correct endpoint if discovery targets /wp-json", () => {
		const prom = WPAPI.discover("http://wpapi.local/wp-json").then((site) => {
			expect(site.posts().toString()).toBe(
				"http://wpapi.local/wp-json/wp/v2/posts"
			);
			return SUCCESS;
	...
	it("resolves to correct endpoint if discovery targets ?rest_route=/", () => {
		const prom = WPAPI.discover("http://wpapi.local/?rest_route=/").then(
			(site) => {
				expect(site.posts().toString()).toBe(
					"http://wpapi.local/wp-json/wp/v2/posts"
				);
