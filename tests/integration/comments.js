...
const getPostsAndAuthors = (comments) =>
	comments.map((comment) => comment.post + comment.author_name);
...
describe.each([
	...
	it("fetches the first page, omitting a password-protected comment", () => {
		const prom = wp
			...
			.get()
			.then((comments) => {
				expect(getPostsAndAuthors(comments)).toEqual(
					expectedResults.postsAndAuthors.page1
				);
				return SUCCESS;
			});
		return expect(prom).resolves.toBe(SUCCESS);
	...
	it('fetches the 10 oldest comments when sorted "asc"', () => {
		const prom = wp
			...
			.then((comments) => {
				expect(getPostsAndAuthors(comments)).toEqual(
					expectedResults.postsAndAuthorsAsc.page1
				);
				return SUCCESS;
			});
		return expect(prom).resolves.toBe(SUCCESS);
			...
			const prom = wp
				...
				.then((posts) => {
					...
					expect(posts._paging.next._options.endpoint).toEqual(
						"http://wpapi.local/wp-json/wp/v2/comments?page=2"
					);
					...
					return wp
						...
						.then((posts) => {
							...
							expect(getPostsAndAuthors(posts)).toEqual(
								expectedResults.postsAndAuthors.page3
							);
							return SUCCESS;
						});
				});
			...
			const prom = wp
				...
				.then((posts) => {
					return posts._paging.next.get().then((posts) => {
						...
						expect(getPostsAndAuthors(posts)).toEqual(
							expectedResults.postsAndAuthors.page2
						);
						return SUCCESS;
					});
				});
				...
				.then((posts) => {
					...
					return posts._paging.next.get().then((posts) => {
						...
						expect(posts._paging.prev._options.endpoint).toEqual(
							"http://wpapi.local/wp-json/wp/v2/comments?page=1"
						);
						return SUCCESS;
					});
				});
		...
		it("allows access to the previous page of results via .prev", () => {
			const prom = wp
				...
				.then((posts) => {
					...
					return posts._paging.prev.get().then((posts) => {
						expect(Array.isArray(posts)).toBe(true);
		...
		beforeEach(() => {
			...
			commentProm = wp
				...
				.then((page) => {
					...
					pageComments = page._embedded.replies.reduce(
						(flatArr, arr) => flatArr.concat(arr),
						[]
					);
					return wp.comments().post(pageId).get();
				});
		});

		it("returns an array of posts", () => {
			const prom = commentProm.then((comments) => {
				expect(Array.isArray(comments)).toBe(true);
				return SUCCESS;
			});
			return expect(prom).resolves.toBe(SUCCESS);
