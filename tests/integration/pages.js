		...
		it("provides a bound WPRequest for the next page as .next", () => {
			const prom = wp
				...
				.then((pages) => {
					...
					expect(pages._paging.next._options.endpoint).toEqual(
						"http://wpapi.local/wp-json/wp/v2/pages?page=2"
					);
		...
		it("provides a bound WPRequest for the previous page as .prev", () => {
			const prom = wp
				...
				.then((pages) => {
					...
					expect(pages._paging.prev._options.endpoint).toEqual(
						"http://wpapi.local/wp-json/wp/v2/pages?page=1"
					);
					return SUCCESS;
				});
			return expect(prom).resolves.toBe(SUCCESS);
			...
			it("can be used to return only pages with the specified slug", () => {
				const prom = wp
					...
					.then((pages) => {
						...
						expect(getTitles(pages)).toEqual(["Clearing Floats"]);
						return SUCCESS;
					});
				return expect(prom).resolves.toBe(SUCCESS);
			});
