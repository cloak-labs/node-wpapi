	...
	describe("paging properties", () => {
		...
		it("provides a bound WPRequest for the next page as .next", () => {
			const prom = wp
				...
				.then((media) => {
					...
					expect(media._paging.next._options.endpoint).toEqual(
						"http://wpapi.local/wp-json/wp/v2/media?page=2"
					);
		...
		it("allows access to the next page of results via .next", () => {
			const prom = wp
				...
				.get()
				.then((media) => {
					return media._paging.next.get().then((media) => {
						expect(Array.isArray(media)).toBe(true);
						expect(media.length).toBe(10);
			...
			const prom = wp
				...
				.then((media) => {
					...
					return media._paging.next.get().then((media) => {
						...
						expect(media._paging.prev._options.endpoint).toEqual(
							"http://wpapi.local/wp-json/wp/v2/media?page=1"
						);
						return SUCCESS;
					});
				});
		...
		it("allows access to the previous page of results via .prev", () => {
			const prom = wp
				...
				.then((media) => {
					expect(getTitles(media)).toEqual(expectedResults.titles.page2);
					return media._paging.prev.get().then((media) => {
						expect(Array.isArray(media)).toBe(true);
						expect(media.length).toBe(10);
						expect(getTitles(media)).toEqual(expectedResults.titles.page1);
						return SUCCESS;
	...
	it("can create, update & delete media when authenticated", () => {
		...
		const prom = authenticated
			...
			.then((createdMedia) => {
				...
				expect(createdMedia.caption.raw).toBe(
					'A painting from Emily Garfield\'s "Conduits" series'
				);

				// File name is correctly applied and image was uploaded to content dir
				expect(imageUrl).toMatch(
					/^http:\/\/wpapi.local\/content\/uploads\/.*\/ehg-conduits.jpg$/
				);
			})
			// UPDATE
			.then(() =>
				authenticated.media().id(id).update({
			...
			.then((result) => {
				...
				expect(result.alt_text).toBe(
					"A photograph of an abstract painting by Emily Garfield"
				);
				return result;
			})
			// READ
			...
			.then((result) => {
				...
				const sizeURLs = objectReduce(
					sizes,
					(urls, size) => urls.concat(size.source_url),
					[]
				);

				// Expect all sizes to have different URLs
				expect(Object.keys(sizes).length).toBe(unique(sizeURLs).length);
				return sizeURLs.reduce(
					(previous, sizeURL) =>
						previous.then(() => httpTestUtils.expectStatusCode(sizeURL, 200)),
					Promise.resolve()
				);
			})
			...
			.then(() => {
				...
				return authenticated.media().id(id).delete();
			})
			.catch((error) => {
