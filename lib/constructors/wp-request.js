...
function WPRequest(options) {
	 ...
	 */
	this._params = {
		...options.globalParams, // Include global parameters
	};
	...
	/**
	 * When true, we return a single record rather than an array
	 *
	 * @property _single
	 * @type boolean
	 * @private
	...
	if (!valid) {
		throw new Error(
			[
				...
				levelDefinitions
					.reduce(
						(components, levelOption) =>
							components.concat(levelOption.component),
						[]
					)
					.join(", "),
			].join(" ")
...
WPRequest.prototype._renderPath = function () {
	...
	const path = [this._namespace]
		.concat(orderedPathParts)
		.filter(identity)
		.join("/");
...
/**
 * Return only provided fields as part of the response payload.
 *
 * @method
 * @chainable
 * @returns {WPRequest} The WPRequest instance (for chaining)
 */
WPRequest.prototype.fields = function (fields) {
	if (fields) return this.param("_fields", fields);
	return this;
};
...
WPRequest.prototype.slug = function (slug) {
	/**
	 * when a slug is provided, we know that a single post object will be returned,
	 * so we default to returning a single object rather than the usual array of posts
	 * with paging information. This prevents needing to chain .slug('xyz').single()
	 */
	this._single = true;

	return paramSetter("slug").call(this, slug);
};

/**
 * Specify that you want to return a single object rather than an array.
 *
 * @method
 * @chainable
 * @returns The request instance (for chaining)
 */
WPRequest.prototype.single = function () {
	this._single = true;
	return this;
};
